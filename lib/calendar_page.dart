...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Твој календар"),),
      body: SfCalendar(
...
List<Appointment> getAppointments(){
 ...
 for(int i=0; i<trueDateList.length; i++) {
   final DateTime startTime = DateTime(trueDateList[i].year, trueDateList[i].month, trueDateList[i].day,
       trueDateList[i].hour, trueDateList[i].minute, trueDateList[i].second);//year, month, day, hour, minute, sec
